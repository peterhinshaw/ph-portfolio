class Item < ActiveRecord::Base
  ...
  def self.description_search_by_category(category, search)
    if search
	   find(:all, :conditions => ['description LIKE ? AND category_id=? AND quantity > 0', "%#{search}%", category])
    else
	   find(:all, :conditions => ['category_id=? AND quantity > 0', category])
    end
  end 
